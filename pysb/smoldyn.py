...
def _get_smoldyn_path():
    """Return the path to Smoldyn's smoldyn, based on either the SMOLDYNHOME
    environment variable if it's set, or a few hard-coded standard locations."""

    path_var = 'SMOLDYNHOME'
    dist_dirs = [
        '/usr/local/bin',
    ...
    def check_dist_dir(dist_dir):
        ...
        # the expected places.
        script_path = os.path.join(dist_dir, 'smoldyn')
    ...
    if path_var in os.environ:
        ...
        if not script_path:
            raise Exception('Environment variable %s is set but smoldyn could'
    ...
    # If the environment variable isn't set, check the standard locations
        ...
        for dist_dir in dist_dirs:
            ...
            raise Exception('Could not find smoldyn installed in one of the '
                            'following locations:' +
                            ''.join('\n    ' + d for d in dist_dirs))
    return script_path

smoldyn_path = _get_smoldyn_path()
